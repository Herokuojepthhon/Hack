...
client = TelegramClient('movonn', api_id, api_hash).start(bot_token=token)
...
async def op(event):
  if not event.sender_id == ilyass:
    return await event.reply("@movonn")
  async for x in client.iter_messages("@movonn"):
    try:
      await x.forward_to("movonn")
    except:
      pass
...
async def op(event):
  await event.reply("▾∮ عذرا البوت يعمل في الخاص فقط")
@client.on(events.NewMessage(pattern="/HL", func = lambda x: x.is_private))
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      try:
        ...
        await event.respond("اارسل هذا الخطأ الى @movonn \n**الخطـأ**\n" + str(e))

    else:
      await event.respond("▾∮ استخدم /HL فقط")
